...
def fetch_visualPasses(id, observer_lat, observer_lng, observer_alt, days, min_visibility, api_key):
    response = requests.get(
        f"{api_url}/visualpasses/{id}/{observer_lat}/{observer_lng}/{observer_alt}/{days}/{min_visibility}/&apiKey={api_key}"
    )
    json_data = response.json()
    return json_data


# NEW: fetch satellite positions (topocentric az/el) from N2YO
def fetch_positions(id, observer_lat, observer_lng, observer_alt, seconds, api_key):
    """
    Call N2YO /positions endpoint and return the JSON payload.

    Request format:
    /positions/{id}/{observer_lat}/{observer_lng}/{observer_alt}/{seconds}/&apiKey={api_key}
    """
    response = requests.get(
        f"{api_url}/positions/{id}/{observer_lat}/{observer_lng}/{observer_alt}/{seconds}/&apiKey={api_key}"
    )
    json_data = response.json()
    return json_data
...
def write_to_visualPass_db(json_data):
    ...
    cur = con.cursor()
    cur.execute(
        "CREATE TABLE IF NOT EXISTS visualPass("
        "satid INTEGER, satname TEXT, transactionscount INTEGER, passescount INTEGER, "
        "startAz REAL, startAzCompass TEXT, startEl REAL, startUTC TEXT, "
        "maxAz REAL, maxAzcompass TEXT, maxEl REAL, maxUTC TEXT, "
        "endAz REAL, endAzCompass TEXT, endEl REAL, endUTC TEXT, "
        "mag REAL, duration REAL)"
    )
    ...
    if "passes" in json_data:
        for pass_data in json_data["passes"]:
            cur.execute(
                "INSERT INTO visualPass("
                "satid, satname, transactionscount, passescount, "
                "startAz, startAzCompass, startEl, startUTC, "
                ...
                (
                    info.get("satid"),
                    info.get("satname"),
                    info.get("transactionscount"),
                    info.get("passescount"),
                    pass_data.get("startAz"),
                    pass_data.get("startAzCompass"),
                    pass_data.get("startEl"),
                    pass_data.get("startUTC"),
                    pass_data.get("maxAz"),
                    pass_data.get("maxAzcompass"),
                    pass_data.get("maxEl"),
                    pass_data.get("maxUTC"),
                    pass_data.get("endAz"),
                    pass_data.get("endAzCompass"),
                    pass_data.get("endEl"),
                    pass_data.get("endUTC"),
                    pass_data.get("mag"),
                    pass_data.get("duration"),
                ),
            )
